...
export function Dialog({ open, onOpenChange, children }: DialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      />
      {/* Content wrapper */}
      <div className="z-50 w-full max-w-lg p-4 animate-in fade-in zoom-in-95 duration-200">
        {children}
      </div>
    </div>
  );
...
export function DialogContent({ className, children, onClose }: { className?: string; children: React.ReactNode; onClose?: () => void }) {
  return (
    <div className={cn("relative rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-6 shadow-lg text-zinc-950 dark:text-zinc-50", className)}>
      {children}
      {onClose && (
        <button 
          onClick={onClose}
